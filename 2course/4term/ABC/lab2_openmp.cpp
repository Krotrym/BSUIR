...
struct Config {
    ...
    int checkpoint = 250;
    int threads = 0;
    std::string out = "lab2_openmp.csv";
};
...
void print_help(const char* exe) {
    std::cout
        ...
        << "  --threads <value>       OpenMP threads, default max available\n"
        << "  --out <file.csv>        output CSV, default lab2_openmp.csv\n";
}
...
Config parse_args(int argc, char** argv) {
    ...
    cfg.threads = get_int(argc, argv, "--threads", cfg.threads);
...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<Result> table(xs.size());

#pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < static_cast<int>(xs.size()); ++i) {
            table[i] = calculate_series(xs[i], cfg.terms, cfg.eps);
        }
        ...
        csv << "elapsed_seconds,iterations,threads,checksum\n";
...
#pragma omp parallel for schedule(dynamic) reduction(+ : batch_sum)
            for (int i = 0; i < batch; ++i) {
                const double x = xs[static_cast<std::size_t>(done + i) % xs.size()];
                batch_sum += calculate_series(x, cfg.terms, cfg.eps).s;
            }

            checksum += batch_sum;
            const auto now = std::chrono::steady_clock::now();
            const std::chrono::duration<double> elapsed = now - start;
            csv << elapsed.count() << ',' << (done + batch) << ','
                << max_threads() << ',' << checksum << '\n';
        }
